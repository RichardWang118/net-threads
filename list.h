...
struct List_s
{
    Node nodes[LIST_MAX_NUM_NODES]; // set array of nodes
    Node *head;                     // head node pointer
    Node *tail;                     // tail node pointer
    Node *current;                  // current node
    List *nextList;                 // next free list pointer
    List *nextNode;                 // next free node pointer
                                    // it is of type list to be used with the corresponding global variable
                                    // it is type casted back to node in the implementation
    int count;                      // counter of nodes in list
};
